...
export class SpotifyAuthGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (access_token == null && refresh_token != null) {
      ...
      const newTokens =
        await this.spotifyAuthService.refreshTokens(refresh_token);

      response.cookie('spotify_access_token', newTokens.access_token, {
        httpOnly: true,
        secure: true,
        expires: new Date(Date.now() + newTokens.expires_in * 1000),
      });
      response.cookie('spotify_refresh_token', newTokens.refresh_token, {
        httpOnly: true,
        secure: true,
      });
