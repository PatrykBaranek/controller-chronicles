...
} from './strategies/reviews-sites-scraper-factory';
...
export class ReviewsSitesService {
  ...
  async getReviewsFromAllSites(gameId: number) {
    ...
    const reviews = await Promise.all(
      reviewSites.map((type) => this.getReviews(game, type)),
    );

    await this.gamesRepository.updateGame(gameId, {
      reviews_sites: reviews.flat(),
    });
  ...
  private async getReviews(game: Game, reviewSite: ReviewsSites) {
    return await this.reviewsSitesScraperFactory
      .createScraper(reviewSite)
      .scrapeData(game);
  }
}
